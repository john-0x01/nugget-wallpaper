# دليل خلفيات Nugget المتحركة

اللغة: [English](./README.md) | [Tiếng Việt](./README.vi.md) | [日本語](./README.ja.md) | [한국어](./README.ko.md) | [ไทย](./README.th.md) | [Русский](./README.ru.md) | [Français](./README.fr.md) | [हिन्दी](./README.hi.md) | [Español](./README.es.md) | **العربية** | [Deutsch](./README.de.md) | [Bahasa Indonesia](./README.id.md)

هذه الصفحة تركز على هدف واحد: استخدام **Nugget** لتفعيل الخلفيات المتحركة على **iPhone / iPad** بدون جيلبريك.

## روابط سريعة
- الصفحة: [https://john-0x01.github.io/nugget-wallpaper/](https://john-0x01.github.io/nugget-wallpaper/)
- مكتبة الخلفيات: [https://wallpaper.nugget.host](https://wallpaper.nugget.host)
- مستودع Nugget: [https://github.com/leminlimez/Nugget](https://github.com/leminlimez/Nugget)

## ما هو Nugget؟
Nugget هو سير عمل لتخصيص أجهزة Apple، ويشمل استيراد الخلفيات المتحركة إلى iPhone و iPad.

## ما هي مكتبة الخلفيات؟
تتوفر المكتبة على [wallpaper.nugget.host](https://wallpaper.nugget.host)، وتوفر موارد قابلة للتنزيل لاستخدامها مع Nugget.

## بدء سريع
1. اختر خلفية متحركة من المكتبة وقم بتنزيلها.
2. افتح Nugget واتبع تعليمات المستودع للاستيراد.
3. فعّل الخلفية من إعدادات iPhone / iPad.

## ملاحظة
هذه صفحة معلومات وليست تابعة لشركة Apple Inc.
