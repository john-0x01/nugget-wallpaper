# Nugget Motion Wallpaper Guide

Language: **English (default)** | [Tiếng Việt](./README.vi.md) | [日本語](./README.ja.md) | [한국어](./README.ko.md) | [ไทย](./README.th.md) | [Русский](./README.ru.md) | [Français](./README.fr.md) | [हिन्दी](./README.hi.md) | [Español](./README.es.md) | [العربية](./README.ar.md) | [Deutsch](./README.de.md) | [Bahasa Indonesia](./README.id.md)

This page focuses on one goal: using **Nugget** to enjoy motion Wallpapers on **iPhone / iPad** without jailbreak.

## Quick Links
- Landing page: [https://john-0x01.github.io/nugget-wallpaper/](https://john-0x01.github.io/nugget-wallpaper/)
- Motion Wallpaper library: [https://mwallx.com](https://mwallx.com)
- Nugget repository: [https://github.com/leminlimez/Nugget](https://github.com/leminlimez/Nugget)

## What Is Nugget?
Nugget is a tool workflow for Apple device customization, including importing motion Wallpaper content to iPhone and iPad.

This page documents a no-jailbreak path for users who want dynamic wallpaper effects without changing low-level system permissions.

## What Is the Wallpaper Library?
The Motion Wallpaper is available at [mwallx.com](https://mwallx.com).  
It provides downloadable motion Wallpaper resources you can use with Nugget.

## Quick Start
1. Open the wallpaper library and download a motion Wallpaper you like.
2. Open Nugget and follow the repository instructions to import the wallpaper.
3. Enable the wallpaper in iPhone / iPad wallpaper settings and check the live effect.

## FAQ
### 1. Do I need jailbreak?
No. This guide is specifically for a no-jailbreak Nugget workflow.

### 2. Which devices are supported?
It mainly targets iPhone and iPad (iOS / iPadOS).  
Check the Nugget repository for detailed compatibility updates.

### 3. Is the Motion Wallpaper the same as Nugget?
No. The Motion Wallpaper provides resources, while Nugget provides the tool and setup workflow.

## Note
This is an informational navigation page for Nugget and Motion Wallpaper resources. It is not affiliated with Apple Inc.
